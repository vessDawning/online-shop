...
public abstract class Wizard extends Stage implements ValueTimestampMaster {
	...
	protected void adjustSizes(final Point2D menuPos) {
		if( menuPos == null) {
			this.centerOnScreen();
		} else {
			this.setX( menuPos.x);
...
class TimestampSelectionPanel extends SimpleSelectionPanel{
	...
	TimestampSelectionPanel( String parameterName, Wizard main) {
		this(parameterName, main, DateAndTime.PARTS_PER_HOUR, 8, 18);
...
//		}
	...
	ReturnValueView( String result, boolean isText, Dimension2D size){
		...
		if( isText){
			...
			textArea.setBackground(new Background( new BackgroundFill(GUIConstants.ReturnValueBackgroundColor, null, null)));
...
class NoteCell extends TreeCell<ViewObjectInTree>{
	...
	public void updateItem( ViewObjectInTree item, boolean empty){
		...
		this.setBackground(new Background( new BackgroundFill( GUIConstants.ReturnValueBackgroundColor, null, Insets.EMPTY)));
		if( item != null ){
